...
def load_build_config(cfg_dir, proj_root):
    ...
    if cfg_dir:
        ...
        cfg_file = os.path.join(proj_root, 'build_central.cfg')
        config['external'] = os.path.abspath(os.path.join(proj_root, os.pardir))
        # substitute_words["PROOT"] = proj_root.replace('\\', '/')
    try:
        fd = open(cfg_file, 'r')
    except IOError:
...
def create_build_command(package, arch, variant, debug, verbose, stage, nr_jobs, generator, cmd_type, config, private_config, cmd):
    ...
    if cmd_type == 'cmake':
        ...
        if stage == 'clean':
            ...
            external_install_dir = os.path.join(config['external'], 'external', 'install')
            add_definition(cmd, 'EXTERNEL_INSTALL', external_install_dir)
